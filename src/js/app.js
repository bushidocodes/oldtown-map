...
class ViewModel {
    constructor() {
        ...
        this.filteredSites = ko.computed(() => {
            if (this.searchString() === '') {
                return $root.sites();
            } else {
                return ko.utils.arrayFilter(this.sites(), (site) => { if (site.name.toLowerCase().indexOf(this.searchString().toLowerCase()) >= 0) return true });
            }
        });
        ...
        this.selectSite = (site) => {
            var markerOfSelectedSite = jQuery.grep(markers(), (marker) => (marker.title === site.name));
            clearInfoWindow();
        ...
        this.highlightMarkerOfSite = (site) => {
            var markerOfSelectedSite = jQuery.grep(markers(), (marker) => (marker.title === site.name));
            markerOfSelectedSite[0].setIcon(highlightedIcon);
        };

        this.unhighlightMarkerOfSite = (site) => {
            var markerOfSelectedSite = jQuery.grep(markers(), (marker) => (marker.title === site.name));
        ...
        this.selectMarker = (marker) => {
            var siteOfSelectedMarker = jQuery.grep(filteredSites(), (site) => (site.name === marker.title));
            clearInfoWindow();
            bounce(marker, 2);
