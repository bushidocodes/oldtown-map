...
var ViewModel = function ViewModel() {
    ...
    // return filered array of all sites that match search string without regard to case
    //Changed syntax from return siteNoCase.includes(filter); to support IE11
    this.filteredSites = ko.computed(function () {
        if (_this.searchString() === '') {
            return $root.sites();
        } else {
            return ko.utils.arrayFilter(_this.sites(), function (site) {
                if (site.name.toLowerCase().indexOf(_this.searchString().toLowerCase()) >= 0) return true;
            });
        }
    });
    ...
    this.selectSite = function (site) {
        ...
        if ($(window).width() < 1500) $("#wrapper").toggleClass("toggled");
    ...
    this.highlightMarkerOfSite = function (site) {
        var markerOfSelectedSite = jQuery.grep(markers(), function (marker) {
            return marker.title === site.name;
        });
    ...
    this.unhighlightMarkerOfSite = function (site) {
        var markerOfSelectedSite = jQuery.grep(markers(), function (marker) {
            return marker.title === site.name;
        });
    ...
    this.selectMarker = function (marker) {
        var siteOfSelectedMarker = jQuery.grep(filteredSites(), function (site) {
...
var my = { viewModel: new ViewModel() };
...
ko.bindingHandlers.updateMarkers = {
    update: function update(element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        for (var i = 0; i < markers().length; i++) {
            ...
            for (var j = 0; j < $root.filteredSites().length; j++) {
                var site = $root.filteredSites()[j];
...
function initMap() {
    ...
    // for (var i = 0; i < my.viewModel.sites().length; i++) {
    //     let site = my.viewModel.sites()[i];
    ...
    //         icon: defaultIcon
    //     });
    //     marker.addListener('click', (event) => my.viewModel.selectMarker(marker));
    //     //Event Listeners for mouseover and mouseout that modify the marker colors upon hover
    //     marker.addListener('mouseover', (event) => this.setIcon(highlightedIcon));
    //     marker.addListener('mouseout', (event) => this.setIcon(defaultIcon));
    //     markers.push(marker);
    // }
}
...
function resolveWikipediaImageURL(site, imageName) {
    $.ajax({
        ...
        dataType: 'jsonp',
        success: function success(data) {
            var imageHTML = '<a href="' + data.query.pages[Object.keys(data.query.pages)].imageinfo[0].url + '">' + '<img class="wiki-img img-circle" src="' + data.query.pages[Object.keys(data.query.pages)].imageinfo[0].url + '" height="20%" width="20%">' + '</a>';
